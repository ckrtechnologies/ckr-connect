...
export const adminLeadsRepository = {
  ...
  async findFiltered(filters = {}) {
    ...
    if (has_followup) {
      conditions.push(`l.next_followup_date IS NOT NULL`);
    }
    ...
    if (tag_id) {
      conditions.push(`l.id IN (SELECT lead_id FROM connect.lead_tags WHERE tag_id = ${idx++})`);
      values.push(tag_id);
    }
    ...
    const query = `
      SELECT 
        ...
        l.source,
        (
          SELECT json_agg(json_build_object('id', t.id, 'name', t.name, 'color_hex', t.color_hex))
  ...
  async countFiltered(filters = {}) {
    const { status, source, assigned_to, tag_id, search, has_followup } = filters;
    const conditions = [];
    const values = [];
    let idx = 1;
  ...
  async findById(id) {
    const query = `
      SELECT 
        ...
        (
          SELECT json_agg(json_build_object('id', t.id, 'name', t.name, 'type', t.type, 'color_hex', t.color_hex))
          FROM connect.lead_tags lt
          JOIN connect.tags t ON lt.tag_id = t.id
          WHERE lt.lead_id = l.id
        ) AS tags
  ...
  async create(data, creatorUserId) {
    ...
    const query = `
      INSERT INTO connect.leads (
        ...
        source, sub_requirement, deal_type, assigned_to,
    ...
    const tagIds = data.tag_ids && data.tag_ids.length > 0 ? data.tag_ids : (data.tag_id ? [data.tag_id] : []);
    if (tagIds.length === 0) {
      // Fallback: assign first active tag
      const tagRes = await db.query('SELECT id FROM connect.tags WHERE is_active = true ORDER BY name ASC LIMIT 1');
      if (tagRes.rows[0]) tagIds.push(tagRes.rows[0].id);
    }
    for (const tid of tagIds) {
      await db.query('INSERT INTO connect.lead_tags (lead_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [lead.id, tid]);
    }
  ...
  /**
   * Manage tags for multiple leads
   */
    ...
    try {
      ...
      for (const leadId of leadIds) {
        if (tagsToRemove && tagsToRemove.length > 0) {
          await client.query(
            'DELETE FROM connect.lead_tags WHERE lead_id = $1 AND tag_id = ANY($2::uuid[])',
            [leadId, tagsToRemove]
          );
        }
        if (tagsToAdd && tagsToAdd.length > 0) {
          for (const tagId of tagsToAdd) {
            await client.query(
              'INSERT INTO connect.lead_tags (lead_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
              [leadId, tagId]
            );
          }
        }
