...
export default function DateSlicePicker() {
  ...
  const [startInput, setStartInput] = useState(dateRange?.startDate || todayStr);
  const [endInput, setEndInput] = useState(dateRange?.endDate || todayStr);

  const mainPresets = [
    { key: 'today', label: 'Today', title: 'Filter by Today' },
    { key: 'mtd', label: 'MTD', title: 'Month to Date' },
    { key: 'this_month', label: 'This Month', title: 'Current Calendar Month' },
    { key: 'last_month', label: 'Last Month', title: 'Previous Calendar Month' },
    { key: 'ytd', label: 'YTD', title: 'Year to Date' },
    { key: 'this_year', label: 'This Year', title: 'Current Calendar Year' },
  ];
  ...
  const setQuickOffset = (days) => {
    ...
    const endIso = `${base.getFullYear()}-${pad(base.getMonth() + 1)}-${pad(base.getDate())}`;
